...
def raytrace(O, D, scene, bounce=0):
    ...
    nearest = func.reduce(np.minimum, distances)
    color = rgb(0, 0, 0)
...
class Sphere:
    ...
    def light(self, O, D, d, scene, bounce):
        ...
        light_nearest = func.reduce(np.minimum, light_distances)
...
scene = [
    Sphere(vec3(.75, .3, 1.), .6, rgb(0, 0, 1)),
...
Image.merge("RGB", rgb).save("fig1.png")
